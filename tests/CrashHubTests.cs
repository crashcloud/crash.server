...
namespace Crash.Server.Tests2
{
	...
	public sealed class CrashHubTests
	{
		...
		public async Task Test_Update_Async(Change change)
		{
			...
			Change newChange = new(change.Id, change.Owner, testPayload) { Type = nameof(Change) };
		...
		public sealed class Changes
		{
			public static IEnumerable TestCases
			{
				get
				{
					yield return new Change(Guid.NewGuid(), "James", "payload") { Type = nameof(Change) };
					yield return new Change(Guid.NewGuid(), "Jenny", "payload") { Type = nameof(Change) };
					yield return new Change(Guid.NewGuid(), "Jeff", "payload") { Type = nameof(Change) };
				}
			}
		}
	}
